...
#define RSS_RING_MAX_READERS 4
     ...
     * rss_ring_report_drop). src_seq_resets counts backward jumps larger
     * than RSS_SRC_SEQ_MAX_GAP (encoder restart, seq domain change): the
...
/* v5 variants that stamp the source frame counter (see src_seq in the slot
 * and the loss counters in the header). src_seq == RSS_SRC_SEQ_NONE skips
 * accounting, identical to the legacy calls above. */
...
/* v5 refmode variant with source frame counter, same semantics as the
 * embedded-mode _seq variants. */
